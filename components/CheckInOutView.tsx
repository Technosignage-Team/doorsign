...
const CheckInOutView: React.FC<CheckInOutViewProps> = ({ onBack, roomStatus }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505] overflow-hidden relative text-white font-display">
      ...
      <div className="absolute top-[-10%] left-[20%] size-[60%] bg-primary/10 blur-[120px] rounded-none pointer-events-none" />
      
      <header className="relative z-30 flex items-center justify-between px-6 py-4 lg:px-16 shrink-0">
        <div className="w-40 lg:w-48">
          <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-white transition-all group px-4 py-2 lg:px-5 lg:py-3 bg-white/5 border border-white/10 rounded-none active:scale-95 backdrop-blur-md">
        ...
        <div className="bg-[#111]/80 p-1.5 rounded-none border border-white/10 flex gap-1 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-3xl ring-1 ring-white/5 transition-opacity duration-300" style={{ opacity: isAuthenticated ? 0.3 : 1, pointerEvents: isAuthenticated ? 'none' : 'auto' }}>
          <button 
            onClick={() => { setActiveTab('GUEST'); handleClear(); }}
            className={`px-6 lg:px-12 py-2.5 lg:py-3 rounded-none text-[9px] lg:text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-500 ${
      ...
      <main className="flex-1 flex flex-row px-6 lg:px-20 gap-8 lg:gap-16 relative z-10 max-w-[1440px] mx-auto w-full pb-8 overflow-y-auto custom-scrollbar">
        ...
        <div className="w-64 lg:w-80 flex flex-col gap-4 py-2 shrink-0 h-full transition-opacity duration-300" style={{ opacity: isAuthenticated ? 0.3 : 1, pointerEvents: isAuthenticated ? 'none' : 'auto' }}>
          <div className="bg-white/5 p-1.5 rounded-none border border-white/5 flex flex-col gap-2 backdrop-blur-lg">
            <button 
              onClick={() => { setMode('CHECK_IN'); handleClear(); }}
              className={`flex items-center gap-3 p-4 rounded-none transition-all duration-500 group relative overflow-hidden ${
                mode === 'CHECK_IN' ? 'bg-primary/20 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'
              }`}
            >
              <div className={`size-9 rounded-none flex items-center justify-center border transition-all ${mode === 'CHECK_IN' ? 'bg-primary text-white border-primary/20' : 'bg-white/5 border-white/5'}`}>
            ...
            >
              <div className={`size-9 rounded-none flex items-center justify-center border transition-all ${mode === 'CHECK_OUT' ? 'bg-primary text-white border-primary/20 shadow-[0_0_20px_rgba(19,127,236,0.3)]' : 'bg-white/5 border-white/5'}`}>
          ...
          <div className="mt-auto flex flex-col gap-4">
            <button 
              ...
              className="w-full bg-[#111] border border-primary/40 text-white py-8 lg:py-10 rounded-none flex flex-col items-center justify-center gap-3 hover:bg-primary/10 hover:border-primary transition-all active:scale-95 group shadow-2xl relative overflow-hidden"
            >
              <div className="size-16 lg:size-20 rounded-none bg-primary/10 border border-primary/20 flex items-center justify-center group-hover:scale-110 transition-transform mb-1">
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-2 overflow-visible">
          <div className="w-full max-w-[520px] bg-white/[0.02] border border-white/10 rounded-none p-6 lg:p-10 backdrop-blur-3xl shadow-[0_40px_120px_rgba(0,0,0,0.8)] flex flex-col items-center relative group min-h-[580px] transition-all duration-500">
            <div className="absolute top-0 right-0 size-64 bg-primary/5 blur-[100px] rounded-none pointer-events-none -translate-y-32 translate-x-32" />
            
            {isAuthenticated ? (
              <div className="w-full flex flex-col items-center animate-in fade-in zoom-in-95 duration-500 py-4">
                 <div className="size-32 lg:size-40 rounded-none bg-emerald-500/20 border-2 border-emerald-500/40 flex items-center justify-center mb-8 shadow-[0_0_60px_rgba(16,185,129,0.3)]">
                   {authenticatedUser?.photo ? (
                     <img src={authenticatedUser.photo} className="size-full object-cover rounded-none" alt="" />
                   ) : (
                 ...
                 <div className="flex flex-col gap-4 w-full px-4">
                    ...
                    <button 
                      onClick={handleClear}
                      className="w-full py-5 rounded-none text-[11px] font-black uppercase tracking-widest text-slate-500 hover:text-white hover:bg-white/5 transition-all"
                    >
                      Cancel & Start Over
                    </button>
                 </div>
              ...
              <>
                <div className="flex flex-col items-center mb-6 text-center relative z-10">
                  <div className="flex items-center gap-2.5 bg-white/5 border border-white/10 px-4 py-1.5 rounded-none mb-6 shadow-xl backdrop-blur-md">
                    <span className="size-1.5 rounded-none bg-emerald-500 animate-pulse shadow-[0_0_10px_#10b981]"></span>
                ...
                <div className="w-full mb-8 relative z-10 flex justify-center gap-3 lg:gap-5 min-h-[80px]">
                  {activeTab === 'GUEST' ? (
                    [0, 1, 2, 3].map((idx) => (
                      <div 
                        key={idx}
                        className={`size-16 lg:size-20 rounded-none border-2 flex items-center justify-center text-2xl font-black transition-all duration-500 ${
                      ...
                      >
                        {inputCode[idx] ? (
                          <div className="size-4 lg:size-5 rounded-none bg-white shadow-[0_0_20px_white] animate-in zoom-in-0 duration-300" />
                        ) : (
                          <div className="size-4 lg:size-5 rounded-none bg-white/[0.02]" />
                        )}
                      </div>
                    ))
                  ) : (
                    <div className="w-full bg-black/60 border-2 border-white/5 rounded-none flex items-center justify-center p-4 shadow-[inset_0_4px_24px_rgba(0,0,0,0.8)] min-h-[80px]">
                ...
                <div className="grid grid-cols-3 gap-3 lg:gap-4 w-full max-w-[340px] mb-2 relative z-10">
                  {['1', '2', '3', '4', '5', '6', '7', '8', '9', 'C', '0', 'delete'].map((key, i) => (
                    <button
                      ...
                      className={`h-14 lg:h-16 flex items-center justify-center rounded-none text-2xl lg:text-3xl font-black transition-all border border-white/5 active:scale-90 shadow-2xl ${
                        key === 'delete' || key === 'C'
            ...
            {error && (
              <div className="mt-4 animate-in fade-in slide-in-from-bottom-2">
                <div className="bg-red-500/25 border border-red-500/40 px-5 py-2 rounded-none text-red-500 text-[9px] font-black uppercase tracking-widest backdrop-blur-md">
                   Access Denied
                </div>
              </div>
            )}
      ...
      {showFaceSurvey && (
        <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-[120px] flex items-center justify-center p-8 animate-in fade-in duration-700">
           ...
           <div className="w-full max-w-4xl bg-[#0b1a2d] border border-white/10 rounded-none p-12 lg:p-20 flex flex-col items-center shadow-[0_0_150px_rgba(0,0,0,1)] relative overflow-hidden">
              ...
              <div className="flex flex-col gap-16 w-full max-w-2xl">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex justify-between items-center bg-white/[0.02] border border-white/5 p-8 rounded-none">
                    <FaceButton 
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[160] bg-black/98 backdrop-blur-[100px] flex items-center justify-center p-8 animate-in fade-in duration-700">
          <div className="bg-[#0b1a2d] w-full max-w-xl rounded-none border border-white/10 p-12 lg:p-16 flex flex-col items-center shadow-2xl relative">
            <div className="size-40 lg:size-48 rounded-none bg-emerald-500/10 border-8 border-emerald-500/20 flex items-center justify-center mb-10 shadow-[0_0_80px_rgba(16,185,129,0.2)]">
            ...
            <button 
              ...
              className="w-full bg-emerald-500 text-white py-8 lg:py-10 rounded-none text-xl lg:text-2xl font-black shadow-2xl shadow-emerald-500/20 hover:brightness-110 active:scale-95 transition-all uppercase tracking-widest border-t border-white/20"
            >
              Finish
            </button>
          </div>
      ...
      {isScanning && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-[60px] flex items-center justify-center p-8 animate-in fade-in duration-500">
           <div className="w-full max-w-4xl h-[80vh] bg-[#0b1a2d] rounded-none border border-white/10 shadow-2xl overflow-hidden flex flex-col">
              <div className="p-8 flex items-center justify-between border-b border-white/5 bg-black/40 z-20">
                ...
                <button onClick={stopScanner} className="size-16 rounded-none bg-white/5 border border-white/10 flex items-center justify-center text-slate-400">
                  <span className="material-symbols-outlined text-4xl">close</span>
                </button>
              </div>
              <div className="relative flex-1 bg-[#050505] overflow-hidden flex items-center justify-center">
                {isInitializing ? (
                  <div className="size-16 border-4 border-primary/20 border-t-primary rounded-none animate-spin"></div>
                ) : (
                  <>
                    <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover opacity-80" />
                    <canvas ref={canvasRef} className="hidden" />
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                      <div className="size-80 border-2 border-primary/30 rounded-none relative">
...
const FaceButton: React.FC<{ 
  ...
  >
    <div className={`size-20 lg:size-24 rounded-none border-2 flex items-center justify-center transition-all duration-500 shadow-xl ${
      active 
