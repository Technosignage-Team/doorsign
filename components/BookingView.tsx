...
const BookingView: React.FC<BookingViewProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505] overflow-hidden relative">
      <div className="absolute top-[-20%] right-[-10%] size-[80%] bg-primary/20 blur-[150px] rounded-none pointer-events-none" />
      <header className="flex items-center justify-between p-1.5 border-b border-white/5 bg-black/20 backdrop-blur-xl relative z-10 shrink-0">
        <div className="flex items-center gap-2">
          <button type="button" onClick={onBack} className="size-7 rounded-none bg-white/5 border border-white/10 flex items-center justify-center text-slate-200 hover:text-white transition-all">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {initialMeetingId && !isPastMeeting && (
                <button 
                  ...
                  className="size-6 rounded-none bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-500 hover:bg-amber-500 hover:text-white transition-all active:scale-95 group shadow-lg"
        ...
        {initialMeetingId && !isPastMeeting && (
          <button onClick={handleDelete} className="px-2 py-1 bg-red-500/10 text-red-500 border border-red-500/20 rounded-none text-[7px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all">
            Cancel
          </button>
        )}
      </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-1.5">
            ...
            <div className="lg:col-span-1 flex flex-col gap-1.5">
              {(initialMeetingId || currentUser) && organizerPhoto && (
                <div className="bg-white/[0.03] border border-white/10 rounded-none p-1.5 flex items-center gap-2 shadow-xl backdrop-blur-md">
                  <div className="size-8 rounded-none bg-white/5 border border-white/10 flex items-center justify-center p-0.5 shrink-0 shadow-inner group relative">
                     <div className="relative size-full overflow-hidden rounded-none">
                    ...
                    <div className="absolute -bottom-1 -right-1 size-3 bg-emerald-500 border-2 border-[#0a0a0a] rounded-none flex items-center justify-center shadow-lg">
              ...
              <section className="flex flex-col gap-1 bg-white/[0.02] border border-white/5 p-1.5 rounded-none">
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-none p-1 text-[10px] font-bold text-white outline-none focus:border-primary color-scheme-dark disabled:opacity-50" 
                  />
                </div>
                {!currentUser && (
                  <div className="flex flex-col gap-0.5">
                    ...
                    <input 
                      ...
                      className={`bg-white/5 border rounded-none p-1 text-[10px] font-bold text-white placeholder:text-slate-500 outline-none transition-all cursor-pointer ${activeInput === 'organizer' ? 'border-primary ring-2 ring-primary/20 bg-white/10' : 'border-white/10 hover:border-white/20'}`} 
                    />
                  </div>
                )}
              </section>
            ...
            <div className="lg:col-span-1 flex flex-col gap-1.5">
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <div className="flex gap-1">
                    <input 
                      ...
                      className={`flex-1 bg-white/5 border rounded-none p-1 text-[10px] font-bold text-white placeholder:text-slate-500 outline-none transition-all cursor-pointer ${activeInput === 'title' ? 'border-primary ring-2 ring-primary/20 bg-white/10' : 'border-white/10 hover:border-white/20'}`} 
                    />
                    <button type="button" disabled={isPastMeeting} onClick={() => setIsSuggestionsOpen(true)} className="size-7 rounded-none bg-[#0b1a2d] border border-primary/20 text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-xl active:scale-95 group shrink-0">
              ...
              <section className="flex flex-col gap-1 bg-white/[0.02] border border-white/5 p-1.5 rounded-none">
                <div className="flex items-center justify-between">
                   ...
                   <button type="button" disabled={isPastMeeting} onClick={() => setIsRecurring(!isRecurring)} className={`size-5 rounded-none flex items-center justify-center transition-all ${isRecurring ? 'bg-primary text-white' : 'bg-white/5 text-slate-400 border border-white/10'}`}>
                  ...
                  <div className="flex flex-col gap-1 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="grid grid-cols-3 gap-1">
                      {['DAILY', 'WEEKLY', 'MONTHLY'].map(f => (
                        <button key={f} type="button" onClick={() => setRecurrence(f as any)} className={`py-0.5 rounded-none text-[5px] font-black transition-all border uppercase tracking-widest ${recurrence === f ? 'bg-primary border-primary text-white shadow-lg' : 'bg-white/5 border-white/10 text-slate-300 hover:text-white hover:border-white/20'}`}>{f}</button>
                      ))}
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <label className="text-slate-300 text-[5px] font-black uppercase tracking-widest ml-1">End Date</label>
                      <input type="date" value={recurrenceEndDate} onChange={(e) => setRecurrenceEndDate(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-none p-1 text-[9px] font-bold text-white outline-none focus:border-primary color-scheme-dark" />
                    </div>
                    {initialMeetingId && (
                      <button 
                        type="button" 
                        onClick={() => setUpdateSeries(!updateSeries)}
                        className={`flex items-center gap-1 p-0.5 rounded-none border transition-all ${updateSeries ? 'bg-primary/10 border-primary/30 text-primary' : 'bg-white/5 border-white/10 text-slate-300'}`}
                      >
              ...
              <section className="flex flex-col gap-1 bg-white/[0.02] border border-white/5 p-1.5 rounded-none">
                ...
                <div className="grid grid-cols-2 gap-1">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <select disabled={isPastMeeting} value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-none p-1 text-[10px] font-bold text-white outline-none focus:border-primary appearance-none">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <select disabled={availableEndOptions.length === 0 || isPastMeeting} value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-none p-1 text-[10px] font-bold text-white outline-none focus:border-primary appearance-none">
                ...
                {error && (
                  <div className="bg-status-busy/20 border border-status-busy/40 rounded-none p-1 flex items-center gap-1 text-status-busy animate-in fade-in slide-in-from-top-2 duration-300">
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-[70] bg-[#0a1016]/95 backdrop-blur-3xl border-t border-white/10 shadow-[0_-40px_120px_rgba(0,0,0,0.9)] transition-all duration-500 ${isKeyboardOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="max-w-4xl mx-auto p-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => { setIsKeyboardOpen(false); setActiveInput(null); }} className="px-4 py-1.5 bg-white/5 hover:bg-white/10 text-slate-200 rounded-none text-[8px] font-black uppercase tracking-widest transition-all">Dismiss</button>
          </div>
          <div className="bg-white/5 border-2 border-primary/30 rounded-none p-4 min-h-[60px] flex items-center relative overflow-hidden">
            ...
            <span className="ml-1 w-0.5 h-8 bg-primary animate-pulse rounded-none"></span>
          </div>
          <div className="flex flex-col gap-1.5">
            {keyboardRows.map((row, rIdx) => (
              <div key={rIdx} className="flex justify-center gap-1.5">
                {rIdx === 2 && <button onClick={() => setIsShift(!isShift)} className={`flex-1 max-w-[80px] h-11 rounded-none flex items-center justify-center transition-all ${isShift ? 'bg-primary text-white shadow-lg' : 'bg-white/5 text-slate-300'}`}><span className="material-symbols-outlined text-xl">shift</span></button>}
                {row.map(key => <button key={key} onClick={() => handleKeyPress(key)} className="flex-1 min-w-[35px] max-w-[70px] h-11 bg-white/5 hover:bg-white/10 active:bg-primary active:text-white border border-white/5 rounded-none text-white text-lg font-black transition-all">{isShift ? key : key.toLowerCase()}</button>)}
                {rIdx === 2 && <button onClick={handleBackspace} className="flex-1 max-w-[80px] h-11 bg-white/5 hover:bg-white/10 rounded-none flex items-center justify-center text-slate-200"><span className="material-symbols-outlined text-xl">backspace</span></button>}
              </div>
            ))}
            <div className="flex justify-center gap-1.5 mt-1">
              <button onClick={handleSpace} className="flex-1 max-w-[400px] h-11 bg-white/5 border border-white/5 rounded-none text-slate-300 text-[10px] font-black uppercase tracking-widest">Space</button>
              <button onClick={() => { setIsKeyboardOpen(false); setActiveInput(null); }} className="w-20 h-11 bg-emerald-500/10 border border-emerald-500/20 rounded-none text-emerald-500 text-[10px] font-black uppercase tracking-widest">Confirm</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {isServiceModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
           ...
           <div className="relative w-full max-w-2xl bg-[#1c2127] rounded-none border border-white/10 shadow-[0_0_100px_rgba(0,0,0,1)] p-10 lg:p-14 flex flex-col items-center gap-8 animate-in zoom-in-95 duration-300 overflow-hidden">
              ...
              {requestStatus === 'SUCCESS' ? (
                <div className="flex flex-col items-center gap-6 py-10 animate-in fade-in zoom-in duration-500">
                   <div className="size-32 rounded-none bg-emerald-500/10 border-4 border-emerald-500/30 flex items-center justify-center text-emerald-500 shadow-xl">
      ...
      {isSuggestionsOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-2xl bg-[#1c2127] rounded-none border border-white/10 shadow-[0_0_100px_rgba(0,0,0,1)] p-10 flex flex-col gap-8 animate-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-2 gap-3">
              {SUGGESTED_TITLES.map(t => (
                <button 
                  ...
                  className="p-5 rounded-none bg-white/5 border border-white/10 text-left hover:bg-primary/10 hover:border-primary transition-all active:scale-95 group"
                >
...
const ServiceCard: React.FC<{ icon: string; title: string; isPending: boolean; onClick: () => void }> = ({ icon, title, isPending, onClick }) => (
  <button onClick={onClick} disabled={isPending} className={`p-6 rounded-none border-2 transition-all flex flex-col items-center gap-4 group h-full relative ${isPending ? 'bg-amber-500/10 border-amber-500/40 shadow-xl' : 'bg-white/5 border-white/10 hover:border-amber-500/50 hover:bg-amber-500/5 active:scale-95'}`}>
    <div className={`size-16 rounded-none flex items-center justify-center transition-all ${isPending ? 'bg-amber-500 text-white' : 'bg-slate-800 text-slate-300 group-hover:text-amber-500'}`}>
       {isPending ? <span className="size-8 border-4 border-white/30 border-t-white rounded-none animate-spin"></span> : <span className="material-symbols-outlined text-3xl font-variation-fill">{icon}</span>}
    </div>
    <div className="text-center">
